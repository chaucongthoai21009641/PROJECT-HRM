...
const LichLamViec = () => {
  ...
  const handleDeleteMultipleShifts = async () => {
    try {
      for (let shiftId of selectedShifts) {
        await axios.delete(
          `${API_BASE}/lichlamviec/deletelichlamviec.php/${shiftId}`
        );
      }
  ...
  const showAssignModal = async () => {
    try {
      const [shiftsRes, employeesRes] = await Promise.all([
        axios.get(`${API_BASE}/calam/calam.php`),
        axios.get(`${API_BASE}/nhanvien/nhanvien.php`),
      ]);
  ...
  const handleAssignShift = async (values) => {
    try {
      ...
      const { data: shiftDetails } = await axios.get(
        `${API_BASE}/calam/getcalamdetail.php/${maCL}`
      );
      ...
      shiftDetails.chiTietCaLams.forEach((detail) => {
        ...
        workingDates.forEach((ngayLamViec) => {
          ...
          requests.push(
            axios.post(
              `${API_BASE}/lichlamviec/insertlichlamviec.php`,
              requestData
            )
          );
        });
  ...
  const fetchEmployees = async () => {
    const res = await axios.get(`${API_BASE}/nhanvien/nhanvien.php`, {
  ...
  const fetchShifts = async () => {
    const res = await axios.get(`${API_BASE}/lichlamviec/lichlamviec.php`, {
      // headers: { "X-CSRF-TOKEN": getCsrfToken() },
    });
    setShifts(res.data);
  };
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editingShift.maLLV) {
        await axios.put(
          `${API_BASE}/lichlamviec/updatelichlamviec.php/${editingShift.maLLV}`,
          payload
        );
  ...
  const deleteScheduleTimeShiftHandle = async (values) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(
            `http://localhost/prjhrm_react_php/backend/api/lichlamviec/deletelichlamviec.php/${editingShift.maLLV}`
          );
  ...
  const scrollToToday = () => {
    ...
    // Nếu ngày hiện tại không nằm trong tháng đang hiển thị thì không làm gì cả
    if (!today.startsWith(month)) return;

    const columnIndex = getAllDaysInMonth(month).findIndex((d) => d === today);

    // Bỏ qua cột đầu tiên (nhân viên), nên cộng thêm 1 vào chỉ số column
