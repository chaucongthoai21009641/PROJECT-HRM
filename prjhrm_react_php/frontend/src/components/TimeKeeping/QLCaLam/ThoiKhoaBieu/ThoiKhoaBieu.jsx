...
const ThoiKhoaBieu = () => {
  ...
  //   try {
  //     const { data } = await axios.get(`${BASE_URL}calam/calam.php`, {
  //       headers,
  //     });

  //     // data chắc chắn là mảng hoặc rỗng
  //     const shifts = (Array.isArray(data) ? data : []).map((shift) => ({
  //       id: shift.maCL,
  //       type: shift.tenCa,
  ...
  const fetchShiftData = async () => {
    try {
      const response = await axios.get(`${BASE_URL}calam/calam.php`, {
        headers,
      });

      if (response.data.status === "success") {
        const shiftList = response.data.data;
  ...
  const fetchTimeDetails = async () => {
    try {
      const { data } = await axios.get(
        `${BASE_URL}chitietcalam/chitietcalam.php`,
        { headers }
      );
      setTimeDetails(data);
    } catch (err) {
  ...
  const handleAddShift = async (values) => {
    try {
      ...
      const calamRes = await axios.post(
        `${BASE_URL}calam/insertcalam.php`,
        {
      ...
      const requests = values.workDays.map((thuTrongTuan) =>
        axios.post(
          `${BASE_URL}chitietcalam/insertchitietcalam.php`,
          {
            thuTrongTuan,
            tgBatDau: startTime,
            tgKetThuc: endTime,
  ...
  const handleEditShiftSubmit = async (updatedValues) => {
    try {
      ...
      const response = await axios.post(
        `${BASE_URL}calam/updatecalam.php/${updatedValues.id}`,
        updatedShift,
        { headers }
      );
    ...
    try {
      if (activeModal === "addTime") {
        ...
        const response = await axios.put(
          `${BASE_URL}chitietcalam/updatechitietcalam.php/${editingTimeShift.maCTCL}`,
          payload,
          { headers }
        );
        console.log(
      ...
      onOk: async () => {
        try {
          const response = await axios.delete(
            `${BASE_URL}chitietcalam/deletechitietcalam.php/${editingTimeShift.maCTCL}`,
            { headers }
          );
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.post(
        `${BASE_URL}calam/deletecalam.php`,
        {
