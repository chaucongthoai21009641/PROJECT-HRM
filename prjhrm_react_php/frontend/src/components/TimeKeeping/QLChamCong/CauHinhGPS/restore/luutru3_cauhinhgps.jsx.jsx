...
const CauHinhGPS = () => {
  ...
  const fetchSuggestions = async (value) => {
    ...
    // Nếu muốn gợi ý trong khu vực VN dùng api này:

    // `https://nominatim.openstreetmap.org/search?format=json&countrycodes=vn&q=${encodeURIComponent(value)}`

    // Giới hạn (nhẹ hơn) `https://nominatim.openstreetmap.org/search?format=json&limit=5&countrycodes=vn&q=${encodeURIComponent(value)}`
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&limit=5&countrycodes=vn&q=${encodeURIComponent(
          value
        )}`
      );
      const data = await response.json();
  ...
  const handleSelectPlace = (value, option) => {
    ...
    form.setFieldsValue({
      viDo: lat.toFixed(6),
      kinhDo: lon.toFixed(6),
    });

    setCurrentLocation({ lat, lng: lon });
    message.success("Đã chọn địa điểm!");
  };
