...
const CauHinhGPS = () => {
  ...
  const loadDiaDiem = async () => {
    try {
      // getDSDiaDiem(url)
      const res = await axios.get(`${BASE_URL}diadiem/gps.php`, { headers });
  ...
  const toggleStatus = async (id, status) => {
    ...
    try {
      await axios.put(
        `${BASE_URL}diadiem/updategps.php`,
        {
          maDD: id,
          trangThai: newStatus,
        },
  ...
  const handleDelete = (id) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`${BASE_URL}diadiem/deletegps.php/${id}`, {
            headers,
          });
          ...
          // showNotification()
          message.error("Có lỗi xảy ra. Không thể xóa địa điểm!");
          console.error(error);
        }
      },
    });
  ...
  const handleSubmit = async (values) => {
    ...
    // requestUpdateDiaDiem()
    // requestAddDiaDiem()
    const url = editMode
      ? `${BASE_URL}diadiem/updategps.php`
      : `${BASE_URL}diadiem/insertgps.php`;
            ...
            >
              ...
              <Form.Item>
                <Button type="primary" htmlType="submit">
                  {editMode ? "Cập nhật" : "Thêm địa điểm"}
                </Button>
              </Form.Item>
            </Form>
          </Card>
