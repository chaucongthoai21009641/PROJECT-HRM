...
const BangChamCong = () => {
  ...
  const stringToColor = (str) => {
    ...
    const color = `hsl(${hash % 360}, 70%, 50%)`; // dùng HSL cho màu đẹp hơn
    return color;
  };
    ...
    const columns = [
      {
        ...
        fixed: "left",
        render: (_, record) => {
          const initials = record.hoTen
            .split(" ")
            .slice(-1)[0] // lấy chữ cuối cùng (tên) để lấy chữ cái đầu
            .charAt(0)
            .toUpperCase();

          const randomBg = stringToColor(record.hoTen); // tạo màu nền từ tên
