...
export default function Information() {
  ...
  const [lichHomNay, setLichHomNay] = useState(null);
  ...
  //     // 1. Lấy lịch làm việc hôm nay của nhân viên
  ...
  //     const lichHienTai = lichLamViec.find(
  //       (lich) =>
  ...
  //     const maLLV = lichHienTai.maLLV;

  //     // 2. Lấy dữ liệu bảng công
  ...
  //     const bangCongHienTai = bangCongList.find((bc) => bc.maLLV === maLLV);
  ...
  //       return resInsert.status === 200
  //         ? notifySuccess("Check-in thành công!")
  //         : notifyError("Check-in thất bại!");
  //     }

  //     if (!bangCongHienTai.tgCheckIn) {
  //       // 🔄 Có bản ghi nhưng chưa check-in => cập nhật check-in
  //       const payload = {
  //         tgCheckIn: currentTime,
  //         tgCheckOut: null,
  //         maLLV,
  ...
  //         `${BASE_URL}bangcong/updatebangcong.php/${bangCongHienTai.maBC}`,
  //         payload,
  //         { headers }
  //       );

  //       return resUpdate.status === 200
  //         ? notifySuccess("Check-in thành công!")
  //         : notifyError("Check-in thất bại!");
  //     }

  //     if (!bangCongHienTai.tgCheckOut) {
  //       // ✅ Đã check-in nhưng chưa check-out => cập nhật check-out
  //       const payload = {
  //         tgCheckIn: bangCongHienTai.tgCheckIn,
  ...
  const handleChamCong = async () => {
    try {
      ...
      // 5. Xử lý các trường hợp chấm công
      if (!bangCongHienTai) {
        const payload = {
          maLLV,
          tgCheckIn: currentTime,
        ...
        <Card hoverable>
          ...
          {lichHomNay && (
            <Card size="small" style={{ marginTop: 16 }}>
              ...
              <Text>Ca: {lichHomNay.tenCa}</Text>
              <br />
              <Text>
                🕗 {lichHomNay.tgBatDau} - {lichHomNay.tgKetThuc}
              </Text>
              <br />
              <Text>
                🛑 Nghỉ: {lichHomNay.tgBatDauNghi} - {lichHomNay.tgKetThucNghi}
              </Text>
            </Card>
          )}
        </Card>
