...
export default function Information() {
  ...
  const handleChamCong = async () => {
    try {
      ...
      let coCaTrongKhoangThoiGian = false;

      for (const lich of lichHomNay) {
        const maLLV = lich.maLLV;

        // ✅ Kiểm tra thời gian hiện tại có nằm trong tgCheckInSom và tgCheckOutMuon
        const nowTime = dayjs(currentTime, "HH:mm:ss");
        const tgCheckInSom = dayjs(lich.tgCheckInSom, "HH:mm:ss");
        const tgCheckOutMuon = dayjs(lich.tgCheckOutMuon, "HH:mm:ss");

        if (nowTime.isBefore(tgCheckInSom) || nowTime.isAfter(tgCheckOutMuon)) {
          continue; // ngoài khung giờ cho phép, bỏ qua
        }

        coCaTrongKhoangThoiGian = true;
      ...
      // ✅ Thông báo phù hợp nếu không chấm được công
      if (!chamCongXong) {
        if (!coCaTrongKhoangThoiGian) {
          notifyAlert("Giờ làm việc chưa bắt đầu hoặc đã kết thúc!");
        } else {
